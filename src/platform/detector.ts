...
export class PlatformDetector {
     ...
     */
    private static async findMSVCCompilerWithEnv(): Promise<{
        path: string
        env: {PATH: string; INCLUDE: string; LIB: string}
    } | null> {
        if (!this.isWindows()) {
            return null
        }
     ...
     */
    private static async getMSVCEnvironment(
        clPath: string,
        vsPath: string
    ): Promise<{PATH: string; INCLUDE: string; LIB: string}> {
        ...
        const pathDirs = [binPath, join(vsPath, 'Common7', 'IDE'), join(vsPath, 'Common7', 'Tools')]

        // Build INCLUDE (C/C++ headers, Windows SDK headers, etc.)
        const includeDirs = [join(msvcRoot, 'include'), join(msvcRoot, 'atlmfc', 'include')]
        ...
        const libDirs = [join(msvcRoot, 'lib', targetArch), join(msvcRoot, 'atlmfc', 'lib', targetArch)]
        ...
        try {
            ...
            if (versions.length > 0) {
                versions.sort().reverse()
                const latestSDK = versions[0]
                libDirs.push(join(kitsLib, latestSDK, 'ucrt', targetArch), join(kitsLib, latestSDK, 'um', targetArch))
            }
        } catch {
            // Continue without SDK paths
        }
